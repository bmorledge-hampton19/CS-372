...
public class VisualCity extends JComponent implements MouseMotionListener, MouseListener {
	 ...
	 * @param g specifies the graphics object that draws stuff.
	 */
	...
	private void displayInfo(Graphics g) {
		...
		if (displayingInfo instanceof Person) {
			...
			g.drawString(String.format("Address: %s", b.getAddress()), 225, 600);
			...
			if (displayingInfo instanceof CityHall) {
				...
				g.drawString(String.format("# of Police: %d", c.getNumberOfOfficers()), 450, 600);
				
				g.drawString("Police: ", 0, 625);
				for (int i = 0; i < c.getNumberOfOfficers(); i++) {
					g.drawString(String.format("%s", c.getOfficer(i).getName()), i*100+100, 625);
				}
				...
				g.drawString(String.format("# of Teachers: %d", s.getNumberOfTeachers()), 450, 600);
				g.drawString(String.format("# of Students: %d", s.getNumberOfStudents()), 575, 600);
				
				g.drawString("Teachers: ", 0, 625);
				for (int i = 0; i < s.getNumberOfTeachers(); i++) {
					g.drawString(String.format("%s", s.getTeacher(i).getName()), i*100+100, 625);
				}
				
				g.drawString("Students: ", 0, 650);
				for (int i = 0; i < s.getNumberOfStudents(); i++) {
					g.drawString(String.format("%s", s.getStudent(i).getName()), i*100+100, 650);
				}
				...
				for (Building b : city.getBuildings()) {
					if (b instanceof CityHall) {
						if (rectangleContainsPoint(e.getX(),e.getY(),b.getImageX(),b.getImageY(),b.getImageWidth(),b.getImageHeight()+100)) {
							...
							// Record that a valid building has been found.
							hasBeenPlaced = true;
							
							// Make sure that this building does not already contain the citizen.
							if (b!=moving.getBuilding()) {
